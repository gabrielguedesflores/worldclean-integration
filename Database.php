...
class Database 
{
    ...
    //busca somente as tarefas filtradas pelo dia de entrega
    public function getTaskFilterExportStatusFalse($hty_id) 
    {
            $dbst = $this->db->prepare("   
            SELECT distinct tsk_id
            , hty_id
            , local.loc_integrationid
            , to_char(tsk_lastexecutiondatehour, 'yyyy-mm-dd') data_ini
            , to_char(tsk_lastexecutiondatehour, 'HH:mm') hora
            , i_observacaototais observation
            from u30219.dbout_history_pedidos
            inner join u30219.local
                on  u30219.dbout_history_pedidos.loc_id = local.loc_id
            where hty_id in ($hty_id)
                                        ");
            return $this->execute($dbst);
    }

    //
    public function getItemTaskFilterHty($hty_id) 
    {
            $dbst = $this->db->prepare("   
            SELECT distinct tsk_id
            , hty_id
            ...
            , to_char(tsk_lastexecutiondatehour, 'HH:mm') hora
            --, i_observacoes 
            --, i_ObservaçãoTotais observation
            from u30219.dbout_history_pedidos
            inner join u30219.local
                on  u30219.dbout_history_pedidos.loc_id = local.loc_id
            where hty_id in ($hty_id)
